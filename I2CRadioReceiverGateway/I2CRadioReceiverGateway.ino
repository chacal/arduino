...
void onI2CReadRequest() {
  if(command == 0) {
    Wire.write(lastMessageLen > 0 ? lastMessageLen + sizeof(lastMessageTime) : 0);  // Append timestamp to all messages
  } else if(command == 1) {
    uint8_t buf[lastMessageLen + sizeof(lastMessageTime)];
    memcpy(buf, lastMessage, lastMessageLen);
    memcpy(buf + lastMessageLen, &lastMessageTime, sizeof(lastMessageTime));
