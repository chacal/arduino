...
typedef enum {
  INA226_CONF_SHUNT_CONTINUOUS     = 0x05,
...
static void on_measurement_timer(void *ctx) {
  ...
  m_measurement_cb(raw_shunt_voltage, shunt_voltage_mv, shunt_current_A);
}
...
void ina226_init(uint32_t measurement_interval_ms, double shunt_resistance, double i_max_expected, ina226_measurement_cb_t callback) {
  ...
  // Initialize INA226. Measure only shunt voltage, conversion time 8ms with 16 sample averaging => new measurement available every ~128ms
  uint16_t config = INA226_CONF_SHUNT_CONTINUOUS | INA226_CONF_SHUNT_CONVERSION_8MS << 3 | INA226_CONF_BUS_CONVERSION_8MS << 6 | INA226_CONF_AVG_16 << 9;
