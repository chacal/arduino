...
std::optional<std::vector<uint8_t>> Util::getAdvPacketField(uint8_t adv_type, const nrf_packet_data *adv_report_packet) {
  const uint8_t *p_data         = &adv_report_packet->payload[6];  // Skip first 6 bytes of adv report (they are a BLE MAC address)
