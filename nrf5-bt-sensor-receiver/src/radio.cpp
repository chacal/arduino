...
void radio_init(radio_packet_cb_t on_rx_adv_packet) {
  ...
  NRF_RADIO->FREQUENCY = channel_resolver_get_frequency(channel_resolver::ADV_CHANNEL_37);
...
static void change_to_next_channel() {
  uint8_t next_channel   = channel_resolver::channel_resolver_get_next_channel();
  NRF_RADIO->FREQUENCY   = channel_resolver::channel_resolver_get_frequency(next_channel);
